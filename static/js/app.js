...
function getTrackColor(id) {
  if (!trackColors[id]) {
    trackColors[id] = TRACK_PALETTE[id % TRACK_PALETTE.length];
  }
...
function playAlert() {
  ...
  if (now - state.lastAlertTime < 1500) {
    return;
  }
...
btnAudioToggle.addEventListener('click', () => {
  ...
  q('icon-audio-on').style.display =
    state.audioEnabled ? '' : 'none';

  q('icon-audio-off').style.display =
    state.audioEnabled ? 'none' : '';

  btnAudioToggle.title =
    state.audioEnabled
      ? 'Matikan audio alert'
...
confSlider.addEventListener('input', () => {
  ...
  vsbConfLabel.textContent =
    `Conf: ${state.confThreshold.toFixed(2)}`;
});

intervalSlider.addEventListener('input', () => {
  state.detectInterval =
    parseInt(intervalSlider.value);

  intervalValue.textContent =
    state.detectInterval + 'ms';
...
function startDrawingZone() {
  ...
  setStatus(
    'Mode gambar zona – klik titik-titik',
    'warning'
  );
}
...
function closeZonePolygon() {
  if (state.zonePoints.length < 3) {
    alert(
      'Minimal 3 titik diperlukan untuk membentuk polygon.'
    );
...
function updateZoneInfo() {
  ...
  zonePointCount.textContent =
    n + ' titik';

  vsbZoneLabel.textContent =
    'Zona: ' + n + ' titik';

  if (state.zoneComplete) {

    zoneStatusText.textContent =
      '✅ Polygon tertutup';
...
zoneCanvas.addEventListener(
  ...
  e => {
    ...
    const rect =
      zoneCanvas.getBoundingClientRect();
...
btnDrawZone.addEventListener(
  'click',
  startDrawingZone
);

btnCloseZone.addEventListener(
  'click',
  closeZonePolygon
);

btnResetZone.addEventListener(
  'click',
  resetZone
);

// ─── CCTV Recording ──────────────────────────────────────────
// Recording dilakukan oleh Flask backend.
// Browser TIDAK menggunakan MediaRecorder.
// Jadi tidak ada download .webm otomatis.
...
function startDetection() {
  ...
  if (!state.zoneComplete) {
    ...
    alert(
      'Gambar dan tutup zona terlebih dahulu sebelum memulai deteksi.'
    );
...
async function stopDetection() {
  ...
  fetch(
    API_BASE + '/tracker/reset',
    {
      method: 'POST'
    }
  ).catch(() => {});

  Object.keys(trackColors)
    .forEach(k => {
      delete trackColors[k];
    });
...
async function runDetectLoop() {
  ...
  try {
    ...
    statPersons.textContent =
      state.personsDetected;

    // ─── Intrusion detected ───────────────────────────────
    if (data.intrusion_detected) {

      const isNewIntrusion =
        !state.intrusion;
  ...
  state.intrFreezeFrames =
    Math.max(
      0,
      state.intrFreezeFrames - 1
    );

  if (
    state.intrFreezeFrames === 0
  ) {
    clearIntrusion();
  }
}
...
btnStartDetect.addEventListener(
  'click',
  startDetection
);

btnStopDetect.addEventListener(
  'click',
  stopDetection
);
...
function addLogItem(
  ...
  if (!item.recordingUrl) {
    waitForLatestRecording(
      item
    );
  }
}

// ─── Find completed CCTV recording ───────────────────────────
  ...
  for (
    ...
    try {
      ...
      if (
        ...
        /*
         * Hanya ambil rekaman yang waktunya
         * dekat dengan event sekarang.
         */
        if (
          !created ||
          created >=
            item.createdAtMs - 10000
        ) {
    ...
    item => {
      ...
      if (
        ...
        recordingStatus.textContent =
          item.recordingPending
            ? '🎥 Rekaman sedang diproses...'
            : '🎥 Rekaman tidak ditemukan.';

        recordingStatus.style.marginTop =
          '6px';
...
function updateHardwareBadge(
  ...
  if (!data.model_loaded) {
    ...
    hwBadgeLabel.textContent =
      'Model tidak termuat';

    hwBadgeDot.style.background =
      '#ef4444';

    hwBadgeDot.style.boxShadow =
      '0 0 6px #ef4444';
...
async function checkServerStatus() {
  ...
  try {
    ...
    if (data.model_loaded) {
      ...
      setStatus(
        'Server aktif · Model gagal dimuat',
        'warning'
      );
    }
    ...
    setStatus(
      'Server tidak terhubung – jalankan app.py',
      ''
    );
  ...
  e => {
    ...
    switch (e.key) {
      ...
      case 'D':
        ...
        if (
          !btnDrawZone.disabled
        ) {

          btnDrawZone.click();

        } else if (
          !btnCloseZone.disabled
        ) {
      ...
      case 'S':
        ...
        if (
          ...
          btnStartDetect.click();

        } else if (
          !btnStopDetect.disabled
        ) {
